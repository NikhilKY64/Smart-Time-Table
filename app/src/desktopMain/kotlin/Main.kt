...
fun main(args: Array<String>) {
    val instancePort = 9991 // Unique port for this app
    
    val repository = TimetableRepository()
    val viewModel = TimetableViewModel(repository)

    try {
        val serverSocket = ServerSocket(instancePort)
        
        // Background listener for second-instance wakeups
        ...
        application {
            ...
            if (isWindowVisible) {
                if (!isOverlayActive) {
                    ...
                    val isSmartHideVisibleByVM by viewModel.isSmartHideVisible.collectAsState()
                    ...
                    LaunchedEffect(isOverlayExpanded, lastInteractionKey, isAutoHideEnabled, isSmartHideVisibleByVM) {
                        if (isOverlayExpanded || isSmartHideVisibleByVM) {
                            handleAlpha = 1f
                            updateInteraction()
                        } else {
                    ...
                    LaunchedEffect(isSmartHideVisibleByVM) {
                        windowState.isMinimized = !isSmartHideVisibleByVM
                    }
                    ...
                    LaunchedEffect(windowState.isMinimized) {
                        if (!windowState.isMinimized && !isSmartHideVisibleByVM) {
                    ...
                    Surface(
                        ...
                        Box(modifier = Modifier.fillMaxSize()) {
                            ...
                            Column(
                                ...
                                val isAutoSlideEnabledByVM by viewModel.isAutoSlideEnabled.collectAsState()
                                Surface(
                                    onClick = { viewModel.setAutoSlideEnabled(!isAutoSlideEnabledByVM) },
                                    shape = RoundedCornerShape(12.dp),
                                    color = if (isAutoSlideEnabledByVM) MaterialTheme.colorScheme.primaryContainer.copy(alpha = 0.4f) else MaterialTheme.colorScheme.surfaceVariant.copy(alpha = 0.4f),
                                    modifier = Modifier.fillMaxWidth()
                                ) {
                                    Row(
                                        ...
                                        Row(verticalAlignment = Alignment.CenterVertically) {
                                            Icon(
                                                ...
                                                tint = if (isAutoSlideEnabledByVM) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant,
                                                modifier = Modifier.size(20.dp)
                                            )
                                            Spacer(Modifier.width(12.dp))
                                            Text("Auto-Slide", fontWeight = FontWeight.Bold)
                                        }
                                        Switch(
        ...
        val isStartup = args.contains("--startup")
        if (!isStartup) {
            try {
                val client = Socket("localhost", instancePort)
                val writer = client.getOutputStream().bufferedWriter()
                writer.write("WAKE_UP_SLIDER\n")
                writer.flush()
                client.close()
            } catch (ex: Exception) {}
        }
